...
<table id="example" class="table table-striped" style="width:100%">
    ...
    <tbody>
        <?php
            $sql = "SELECT * FROM preamble";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    echo "<tr>";
                        echo "<td>" . htmlspecialchars($row['course_code']) . "</td>";
                        echo "<td>" . htmlspecialchars($row['preamble']) . "</td>";
                        echo "<td>";
                            echo "<button class='btn btn-sm btn-primary editBtn' data-id='".$row['id']."' data-code='".$row['course_code']."' data-preamble='".$row['preamble']."'><i class='bi bi-pencil'></i></button>";
    ...
    <div class="modal-content">
        ...
        <form method="post">
            ...
            <div class="modal-footer">
                <button type="save" class="btn btn-success" name="save_preamble">Save</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="modal-content">
        ...
        <form method="post">
            ...
            <div class="form-row">
                ...
                <div class="form-column">
                    <label for="edit_preamble">Preamble:</label>
                    <textarea type="text" name="preamble" id="edit_preamble" required></textarea>
                </div>
            </div>
            <div class="modal-footer">
                <button type="submit" class="btn btn-success" name="update_preamble">Update</button>
            </div>
        </form>
    </div>
</div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form method="POST" action="" id="deleteForm">
                    ...
                    <button type="save" class="btn btn-danger" name="delete_preamble">Delete</button>
                </form>
            </div>
        </div>
    </div>
...
<script>
    ...
    document.querySelectorAll('.editBtn').forEach(function(button) {
        button.onclick = function() {
            ...
            var preamble = this.getAttribute("data-preamble");
            document.getElementById("edit_id").value = id;
            document.getElementById("edit_course_code").value = code;
            document.getElementById("edit_preamble").value = preamble;
