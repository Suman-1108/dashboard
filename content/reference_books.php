...
<table id="example" class="table table-striped" style="width:100%">
    ...
    <tbody>
        <?php
            $sql = "SELECT * FROM reference_book";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    echo "<tr>";
                        echo "<td>" . htmlspecialchars($row['course_code']) . "</td>";
                        echo "<td>" . htmlspecialchars($row['reference_book']) . "</td>";
                        echo "<td>";
                            echo "<button class='btn btn-sm btn-primary editBtn' data-id='".$row['id']."' data-code='".$row['course_code']."' data-reference_book='".$row['reference_book']."'><i class='bi bi-pencil'></i></button>";
    ...
    <div class="modal-content">
        ...
        <form method="post">
            <div class="form-row">
                ...
                <div class="form-column">
                    <label for="reference_books">Reference Book:</label>
                    <input type="text" id="reference_book" name="reference_book" required>
                </div>
            </div>
            <div class="modal-footer">
                <button type="save" class="btn btn-success" name="save_reference_book">Save</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="modal-content">
        ...
        <form method="post">
            ...
            <div class="form-row">
                ...
                <div class="form-column">
                    <label for="edit_reference_book">Text Book:</label>
                    <input type="text" id="edit_reference_book" name="reference_book" required>
                </div>
            </div>
            <div class="modal-footer">
                <button type="submit" class="btn btn-success" name="update_reference_book">Update</button>
            </div>
        </form>
    </div>
</div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form method="POST" action="" id="deleteForm">
                    ...
                    <button type="save" class="btn btn-danger" name="delete_reference_book">Delete</button>
                </form>
            </div>
        </div>
    </div>
...
<script>
    ...
    document.querySelectorAll('.editBtn').forEach(function(button) {
        button.onclick = function() {
            ...
            var reference_book = this.getAttribute("data-reference_book");
            document.getElementById("edit_id").value = id;
            document.getElementById("edit_course_code").value = code;
            document.getElementById("edit_reference_book").value = reference_book;
