...
<table id="example" class="table table-striped" style="width:100%">
    ...
    <tbody>
        <?php
            $sql = "SELECT * FROM web_resources";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    echo "<tr>";
                        echo "<td>" . htmlspecialchars($row['course_code']) . "</td>";
                        echo "<td>" . htmlspecialchars($row['web_resources']) . "</td>";
                        echo "<td>";
                            echo "<button class='btn btn-sm btn-primary editBtn' data-id='".$row['id']."' data-code='".$row['course_code']."' data-web_resources='".$row['web_resources']."'><i class='bi bi-pencil'></i></button>";
    ...
    <div class="modal-content">
        ...
        <form method="post">
            <div class="form-row">
                ...
                <div class="form-column">
                    <label for="web_resources">Web Resources:</label>
                    <input type="text" id="web_resources" name="web_resources" required>
                </div>
            </div>
            <div class="modal-footer">
                <button type="save" class="btn btn-success" name="save_web_resources">Save</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="modal-content">
        ...
        <form method="post">
            ...
            <div class="form-row">
                ...
                <div class="form-column">
                    <label for="edit_web_resources">Text Book:</label>
                    <input type="text" id="edit_web_resources" name="web_resources" required>
                </div>
            </div>
            <div class="modal-footer">
                <button type="submit" class="btn btn-success" name="update_web_resources">Update</button>
            </div>
        </form>
    </div>
</div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form method="POST" action="" id="deleteForm">
                    ...
                    <button type="save" class="btn btn-danger" name="delete_web_resources">Delete</button>
                </form>
            </div>
        </div>
    </div>
...
<script>
    ...
    document.querySelectorAll('.editBtn').forEach(function(button) {
        button.onclick = function() {
            ...
            var web_resources = this.getAttribute("data-web_resources");
            document.getElementById("edit_id").value = id;
            document.getElementById("edit_course_code").value = code;
            document.getElementById("edit_web_resources").value = web_resources;
